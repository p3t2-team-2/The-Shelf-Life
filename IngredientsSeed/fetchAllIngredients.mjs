...
async function fetchAllIngredients() {
    ...
    for (let i = progress.letterIndex; i < alphabet.length; i++) {
        ...
        while (offset < totalResults && requestCount < 150) {
            ...
            requestCount++;
            console.log(`Fetched ${allIngredients.length} ingredients so far (current letter: ${letter}, ${offset + results.length} / ${totalResults}, request #${requestCount})`);
            offset += PAGE_SIZE;
            // Save progress
            fs.writeFileSync(PROGRESS_FILE, JSON.stringify({ letterIndex: i, offset }, null, 2));
